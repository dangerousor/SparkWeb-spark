...
def do_with_task(args, co, db_worker, task_id):
    ...
    data = dict()
    for each in res['in0']:
        result, tmp = run_func(each, params=dic[each]['params'])
        if not result:
            sc.cancelAllJobs()
            return False, tmp
        data[each] = tmp
    ...
    for each in res['in1']:
        ...
        result, tmp = run_func(each, params=dic[each]['params'], in1=data[lines[each][0]])
        if not result:
            sc.cancelAllJobs()
            return False, tmp
        data[each] = tmp
    ...
    for each in res['in2']:
        ...
        result, tmp = run_func(each, params=dic[each]['params'], in1=data[lines[each][0]], in2=data[lines[each][1]])
        if not result:
            sc.cancelAllJobs()
            return False, tmp
        data[each] = tmp
